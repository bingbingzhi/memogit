...
import pandas as pd
from labels5_extract_fr_tense import *
...
print("train:\n",pd.value_counts(train_tense_distr))
print("train:\n",pd.value_counts(test_tense_distr))
...
#wordset2,WPset2 = make_w2idx(no_pres_train_treebank)
#word2idx2 = dict(zip(wordset2,range(len(wordset2))))
#WP2idx2 =dict(zip(WPset2,range(len(WPset2))))
#n_features2 = len(feats_temps)+len(word2idx2)+len(feats_aspect)+len(feats_tmod)+len(feats_adv)+len(feats_md)+len(WP2idx2)+2
#n_features2 = len(WP2idx2)+len(feats_temps)+len(feats_md)+len(word2idx2)
n_features = len(feats_temps)+len(feats_aspect)+len(feats_tmod)+len(feats_adv)+len(feats_md)+len(WP2idx)+2
...
#print("#train_samples: ",len(no_pres_train_treebank))
#print("#dev_samples: ",n_test_samples)
#print("#test_samples: ",len(no_pres_test_treebank))
...
def tree2features(deptree,Pres=True,y_test_Pres=False):
	...
	xfeatures = np.concatenate((x2,F_tmod,F_val))
	#xfeatures = np.concatenate((x3,F_val))
	#x1 = np.concatenate((F_WP,F_context,F_md))
	#xfeatures = np.concatenate((x1,F_VV))
...
def make_samples(dataset,n_samples,n_features,Pres=True,y_test_Pres=False):
	...
	for deptree in dataset:
		xfeatures,y = tree2features(deptree,Pres,y_test_Pres)
		X_matrix[id_sample]=xfeatures
		id_sample += 1
		Y.append(y)
	return X_matrix,np.array(Y)

X_train,y_train = make_samples(train_treebank,len(train_treebank),n_features,Pres=True,y_test_Pres=False)
X_test,y_test = make_samples(test_treebank,len(test_treebank),n_features,Pres=True,y_test_Pres=True)
no_pres_X_train,no_pres_y_train = make_samples(no_pres_train_samples,len(no_pres_train_samples),n_features,Pres=False,y_test_Pres=False)
...
sp.save_npz('../data/binM_no_pres_X_train.npz',no_pres_X_train)
#sp.save_npz('../data/p2_no_pres_X_test.npz',no_pres_X_test)
t1 = time.time() - t
